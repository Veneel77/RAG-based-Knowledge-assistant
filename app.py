...
if not GEMINI_API_KEY:
    GEMINI_API_KEY = os.getenv("GEMINI_API_KEY")
if not GEMINI_API_KEY:
    st.error("GEMINI_API_KEY is not configured. Add it to Streamlit Secrets or your local .env file.")
    st.stop()
...
INDEX_PATH = "faiss.index"
META_PATH = "meta.pkl"
DEFAULT_PDFS = ["docs/AI1.pdf", "docs/aiimpact.pdf"]
TOP_K = 5
...
def create_default_index():
    valid_pdfs = [pdf for pdf in DEFAULT_PDFS if os.path.exists(pdf)]
    if not valid_pdfs:
        return None, []
    try:
        build_index(valid_pdfs, out_index=INDEX_PATH, out_meta=META_PATH)
        return load_index()
    except Exception as e:
        st.error(f"Could not create default knowledge base: {e}")
        return None, []
...
def retrieve(query, k=TOP_K):
    ...
    # Lazy-load the embedding model only when a query is made.
...
with st.sidebar:
    ...
    if uploaded_file is not None and st.session_state.uploaded_pdf != uploaded_file.name:
        with st.spinner("Processing your PDF..."):
            ...
            # Only the user's uploaded PDF is indexed.
            build_index([upload_path], out_index=INDEX_PATH, out_meta=META_PATH)
    ...
    if st.session_state.uploaded_pdf:
        st.success(f"📄 {st.session_state.uploaded_pdf}")
        st.caption("Questions are answered using this uploaded document.")
    else:
    ...
    st.divider()
    st.selectbox("Model", ["Gemini 2.5 Flash"], index=0)

    if st.session_state.active_index is not None:
        st.caption(f"Indexed chunks: {st.session_state.active_index.ntotal}")
...
user_query = st.chat_input("Message Nova AI...")
...
if user_query:
    ...
    if not contexts:
        answer = "I couldn't find relevant information in the current document."
...
Answer the user's question using ONLY the information contained in the retrieved document context.

Rules:
1. Ground your answer in the provided context.
2. Do not invent information.
3. If the answer is not present, say: "I couldn't find this information in the uploaded document."
        ...
        with st.chat_message("assistant"):
            with st.spinner("Thinking..."):
                try:
                    ...
                    answer = f"Gemini encountered an error.\n\n{str(e)}"
